...
from .checks import discover_home, profile_config_check
...
def build_report(mode: str, hermes_home: Path) -> DoctorReport:
    ...
    if mode in {"quick", "full", "post-update"}:
        checks.append(profile_config_check(hermes_home))
    return DoctorReport.build(mode=mode, hermes_home=str(hermes_home), checks=checks)
...
def parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--fail-on", choices=["warn", "fail", "needs-approval"], default=None)
    return p
